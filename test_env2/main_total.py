...
def main():
    ...
    train_df_inputs1 = df_scaled[['PI', 'Do', 'intensity', 'exposure_time']]
    train_df_targets1 = df_scaled[['cured_height']]
    train_dataset1 = MyDataset(train_df_inputs1, train_df_targets1)
    
    train_df_inputs2 = df_scaled[['intensity', 'exposure_time', 'cured_height']]
    ...
    ##### Model1 Training #####
    optimizer_ft = optim.Adam(model1.parameters(), lr=0.001, weight_decay=0.01)
    ...
    ##### Model2 Training #####
    optimizer_ft = optim.Adam(model2.parameters(), lr=0.001, weight_decay=0.01)
    exp_lr_scheduler = optim.lr_scheduler.StepLR(optimizer_ft, step_size=5, gamma=0.5)
    criterion = nn.MSELoss()
