...
def log_regression_with_Do_DNN():
    ...
    for name, group in grouped:
        ...
        ##### Test Test Test Test #####
        third_from_end = x_data_filtered[-3]
        tenth_from_end = x_data_filtered[-10]
        fifteenth_from_end = x_data_filtered[-15]

        x_data_filtered = [third_from_end, tenth_from_end, fifteenth_from_end]
        ##### Test Test Test Test #####
        ...
        if count == 3:
            ...
            for i in range(3):
                ...
                exp_inter = scaler.inverse_transform([[x_data_filtered[i]]])
                scaler = scalers['cured_height']
                ch_inter = scaler.inverse_transform([[y_pred_filtered[i]]])
                print(x_data_filtered[i])
                print(y_pred_filtered[i])
                
                print(name)
            
            cd2 = train_dataset.input[flattened_index].view(-1, 3)
            
            # for candidate in cd2:
            #     scaler = scalers['intensity']
            #     scaler = scalers['exposure_time']
            #     scaler = scalers['cured_height']
            #     print(candidate[0].numpy())
            #     print(candidate[1].numpy())
            #     print(candidate[2].numpy())
            #     print('\n')
